...
- **`max_steps` resolution.** Explicit `--max-steps N` CLI > explicit `training.max_steps` in config > `data.source_max_steps[source]` for AttCT runs > `None` (full epoch). **BCT (SFTLoss) runs skip the source_max_steps cap entirely** (BCT trains from `data.bct_root`, not from a `source`); this prevents the silent-undertraining bug where `clear-harm: 179` was leaking into BCT runs.

...
- **`attct_datasets.py`** — `get_dataloader()` for AttCT/ACT/MLPCT (clean + wrapped pairs); `get_bct_dataloader()` for BCT (clean prompt → clean response). Owns the longest-matching-suffix token alignment that handles BPE context-sensitivity between clean and wrapped tokenizations (paper §3.2 AttCT method).

...
Per-model headline launchers used in the paper. Each invokes `run_act.sh` / `run_bct.sh` (or the jailbreak filter pipeline) with that model's best config and the W&B `RUN_GROUP` / `HF_REPO` conventions used in the paper.
